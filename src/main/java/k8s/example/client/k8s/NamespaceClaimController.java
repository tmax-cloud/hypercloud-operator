		...
		try {
			nscController.forEach(response -> {
				...
				try {
					...
					if( claim != null) {
						...
						switch( eventType ) {
							case Constants.EVENT_TYPE_ADDED : 
								if ( claim.getMetadata().getLabels() == null || claim.getMetadata().getLabels().get( Constants.NAMESPACE_OWNER_LABEL ) == null || claim.getMetadata().getLabels().get( Constants.NAMESPACE_OWNER_LABEL ).isEmpty() ) {
									System.out.println( "[NamespaceClaim Controller][Exception] label 'ownerUserName' is empty. Reject this Claim." );
									replaceNscStatus( claim.getMetadata().getName(), Constants.NAMESPACE_CLAIM_STATUS_REJECT, "label 'ownerUserName' is empty." );
								}
												
								String ownerUserName = claim.getMetadata().getLabels().get( Constants.NAMESPACE_OWNER_LABEL );
								System.out.println("[NamespaceClaim Controller] Owner User Name : " + ownerUserName );
								
								// Patch Status to Awaiting
								replaceNscStatus( claim.getMetadata().getName(), Constants.NAMESPACE_CLAIM_STATUS_AWAITING, "wait for admin permission" );
								break;
							case Constants.EVENT_TYPE_MODIFIED : 
								String status = getClaimStatus( claim.getMetadata().getName() );
								if ( status.equals( Constants.NAMESPACE_CLAIM_STATUS_SUCCESS ) && !K8sApiCaller.namespaceAlreadyExist( claimName ) ) {
									K8sApiCaller.createNamespace( claim );
									K8sApiCaller.ownerRoleBinding( claim );
									replaceNscStatus( claim.getMetadata().getName(), Constants.NAMESPACE_CLAIM_STATUS_SUCCESS, "namespace create success." );
								} else if ( ( status.equals( Constants.NAMESPACE_CLAIM_STATUS_AWAITING ) || status.equals( Constants.NAMESPACE_CLAIM_STATUS_REJECT ) ) && K8sApiCaller.namespaceAlreadyExist( claimName ) ) {
									replaceNscStatus( claim.getMetadata().getName(), Constants.NAMESPACE_CLAIM_STATUS_SUCCESS, "namespace create success." );
								}
					...
					try {
						replaceNscStatus( claimName, Constants.NAMESPACE_CLAIM_STATUS_ERROR, e.getMessage() );
